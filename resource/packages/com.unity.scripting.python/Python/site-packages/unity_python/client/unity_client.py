...
import rpyc
from unity_python.server.settings import unity_server_port
...
def connect(Service = UnityClientService, port = unity_server_port):
    """
    Create a thread that connects to Unity on the given port.

    Return the connection.
    """
    config = { 'propagate_SystemExit_locally': True, 'propagate_KeyboardInterrupt_locally': True }
    connection = rpyc.connect('localhost', port, Service, config = config)
