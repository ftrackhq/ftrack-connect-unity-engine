...
import rpyc
from settings import (unity_server_port, process_jobs_max_batch_time)
...
def start_server():
    ...
    print("Unity Python server: main thread is {}".format(main_thread_id))

    global clients
    clients = dict()

    config = { 'allow_public_attrs': True, 'allow_setattr': True }
    server = rpyc.ThreadedServer(UnityService, port = unity_server_port, protocol_config = config)
...
def spawn_subpython(args, wantLogging = True, python_executable = sys.executable, env_override = {}):
    ...
    if wantLogging:
        utils.log('PythonNet: Creating client process: {} with PYTHONPATH={}'.format(' '.join(args), client_environment['PYTHONPATH']))
    try:
